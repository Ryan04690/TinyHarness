...
result = agent.run(
    (
        "Use the shell to check whether a file "
        "named definitely_not_exists.txt exists. "
        "If the command fails, inspect the result "
        "and explain what happened."
    )
)

# The corresponding CMD command is if exist definitely_not_exists.txt (echo FOUND) else (echo MISSING)
